...
namespace Live2D.Cubism.Framework.Expression
{
	...
	/// </summary>
	public class CubismExpressionController : MonoBehaviour, ILateUpdatable, IEnable
	{
		#region variable
		...
		private void UpdateExpression()
		{
			...
			for (var expressionIndex = 0; expressionIndex < _playingExpressions.Count; ++expressionIndex)
			{
				...
				expressionWeight += playingExpression.FadeInTime == 0.0f ? 1.0f : CubismFadeMath.GetEasingSine((playingExpression.ExpressionUserTime - playingExpression.ExpressionStartTime) / playingExpression.FadeInTime);
			}

			// ----- If the latest Expression fade is complete, delete the earlier one. ------
			if (_playingExpressions.Count > 1 && _playingExpressions[_playingExpressions.Count - 1].FadeWeight >= 1.0f)
			{
			...
			for (var i = 0; i < _expressionParameterValues.Count; i++)
			{
				...
				expressionParameterValue.Parameter.BlendToValue(CubismParameterBlendMode.Override, (expressionParameterValue.OverwriteValue + expressionParameterValue.AdditiveValue) * expressionParameterValue.MultiplyValue, expressionWeight);
		...
		private void UpdateFadeWeight(CubismPlayingExpression playingExpression)
		{
			...
			// Update weight
			playingExpression.FadeInWeight = (Mathf.Abs(playingExpression.FadeInTime) < float.Epsilon) ? 1.0f : CubismFadeMath.GetEasingSine(playingExpression.ExpressionUserTime / playingExpression.FadeInTime);

			playingExpression.FadeOutWeight = ((Mathf.Abs(playingExpression.ExpressionEndTime) < float.Epsilon) || (playingExpression.ExpressionEndTime < 0.0f)) ? 1.0f : CubismFadeMath.GetEasingSine((playingExpression.ExpressionEndTime - playingExpression.ExpressionUserTime) / playingExpression.FadeOutTime);
		...
		private void CalculateExpressionParameters(int expressionIndex, CubismPlayingExpression playingExpression)
		{
			...
			for (var i = 0; i < _expressionParameterValues.Count; i++)
			{
				...
				var currentParameterValue = expressionParameterValue.OverwriteValue = expressionParameterValue.Parameter.Value;
				...
				if (parameterIndex < 0)
				{
					...
					{
						expressionParameterValue.AdditiveValue = CalculateValue(expressionParameterValue.AdditiveValue, DefaultAdditiveValue, playingExpression.FadeWeight);
						expressionParameterValue.MultiplyValue = CalculateValue(expressionParameterValue.MultiplyValue, DefaultMultiplyValue, playingExpression.FadeWeight);
						expressionParameterValue.OverwriteValue = CalculateValue(expressionParameterValue.OverwriteValue, currentParameterValue, playingExpression.FadeWeight);
					}
				...
				{
					expressionParameterValue.AdditiveValue = CalculateValue(expressionParameterValue.AdditiveValue, newAdditiveValue, playingExpression.FadeWeight);
					expressionParameterValue.MultiplyValue = CalculateValue(expressionParameterValue.MultiplyValue, newMultiplyValue, playingExpression.FadeWeight);
					expressionParameterValue.OverwriteValue = CalculateValue(expressionParameterValue.OverwriteValue, newSetValue, playingExpression.FadeWeight);
				}
