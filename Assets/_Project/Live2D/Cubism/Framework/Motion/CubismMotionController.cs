...
namespace Live2D.Cubism.Framework.Motion
{
	...
	public class CubismMotionController : MonoBehaviour, IEnable, IDisable, IUpdatable
	{
		#region Action
		...
		{
			// Fail silently...
			if (!enabled || !_isActive || _cubismFadeMotionList == null || clip == null || layerIndex < 0 || layerIndex >= LayerCount || ((_motionPriorities[layerIndex] >= priority) && (priority != CubismMotionPriority.PriorityForce)))
			{
