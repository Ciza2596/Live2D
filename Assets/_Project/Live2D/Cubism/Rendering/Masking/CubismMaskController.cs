...
namespace Live2D.Cubism.Rendering.Masking
{
	...
	public sealed class CubismMaskController : MonoBehaviour, ICubismMaskTextureCommandSource, ILateUpdatable, IStartable, IDestroyable
	{
		/// <summary>
		...
		{
			var drawables = this.FindCubismModel().Drawables;
			...
			for (var i = 0; i < Junctions.Length; ++i)
			{
				...
				// Create junction.
				Junctions[i] = new CubismMaskMaskedJunction().SetMasks(masks).SetMaskeds(pairs.Entries[i].Maskeds.ToArray()).SetMaskTexture(MaskTexture);
			}
		}
